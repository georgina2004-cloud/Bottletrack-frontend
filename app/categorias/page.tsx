...
function CategoriasContent() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      try {
        ...
        if (!isMounted) return;
        setErrorCarga(err instanceof Error ? err.message : "No se pudieron cargar las categorías del servidor.");
      } finally {
  ...
  const tieneColumnaConteo = useMemo(() => categorias.some((c) => typeof c.productos_count === "number"), [categorias]);
  ...
  const handleConfirmarEliminar = async () => {
    ...
    try {
      ...
      setSuccessMessage(`La categoría "${categoriaParaEliminar.nombre}" ha sido eliminada correctamente.`);
      setCategorias((prev) => prev.filter((c) => c.id !== categoriaParaEliminar.id));
      if (categoriaSeleccionada?.id === categoriaParaEliminar.id) setCategoriaSeleccionada(null);
      setCategoriaParaEliminar(null);
    } catch (err: unknown) {
      setErrorMessage(err instanceof Error ? err.message : "No se pudo eliminar la categoría.");
      setCategoriaParaEliminar(null);
    } finally {
      setIsDeleting(false);
    }
      ...
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="p-2.5 rounded-xl bg-brand/10 text-brand"><Layers className="w-6 h-6" /></div>
                <div>
                  <h1 className="text-2xl font-bold tracking-tight text-[#18181B] font-sans">Categorías</h1>
                  <p className="text-xs text-slate-500 mt-0.5">Haz clic en una categoría para ver sus detalles y acciones.</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={handleManualRefresh} disabled={isLoading || isRefreshing} className="text-xs gap-1.5 shadow-2xs" title="Recargar categorías">
                  <RefreshCw className={`w-3.5 h-3.5 text-slate-500 ${isRefreshing ? "animate-spin text-brand" : ""}`} />
                ...
                {tienePermisoEscritura && (
                  <Link href="/categorias/nuevo">
                    <Button size="sm" className="text-xs gap-1.5 shadow-sm shadow-brand/20">
                      <Plus className="w-3.5 h-3.5" /><span>Nueva Categoría</span>
                    </Button>
                  </Link>
                )}
              </div>
          ...
          {effectiveSuccessMessage && (
            <div role="alert" className="p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs sm:text-sm flex items-center justify-between gap-3 shadow-2xs animate-in fade-in duration-200">
              <div className="flex items-center gap-2.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                <p className="font-medium">{effectiveSuccessMessage}</p>
              </div>
              <button type="button" onClick={handleDismissSuccess} className="text-emerald-500 hover:text-emerald-800 p-1 rounded-lg hover:bg-emerald-100/50 transition-colors cursor-pointer"><X className="w-4 h-4" /></button>
            </div>
          )}
          {effectiveErrorMessage && (
            <div role="alert" className="p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs sm:text-sm flex items-center justify-between gap-3 shadow-2xs animate-in fade-in duration-200">
              <div className="flex items-center gap-2.5">
                <AlertTriangle className="w-4 h-4 text-rose-600 shrink-0" />
                <p className="font-medium">{effectiveErrorMessage}</p>
              </div>
              <button type="button" onClick={handleDismissError} className="text-rose-400 hover:text-rose-700 p-1 rounded-lg hover:bg-rose-100/50 transition-colors cursor-pointer"><X className="w-4 h-4" /></button>
            </div>
          )}
          ...
          <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="relative flex-1">
              ...
              />
              {busqueda && (
                <button type="button" onClick={() => setBusqueda("")} className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 cursor-pointer"><X className="w-3.5 h-3.5" /></button>
              )}
            </div>
            ...
            {busqueda && (
              <span className="text-xs text-slate-500 shrink-0 font-medium">{categoriasFiltradas.length} resultado{categoriasFiltradas.length === 1 ? "" : "s"}</span>
            )}
          </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50/80 border-b border-slate-200/80 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                      ...
                      {tieneColumnaConteo && <th className="py-3.5 px-4 text-center">Productos</th>}
                    </tr>
                  </thead>
                    ...
                    {isLoading ? (
                      Array.from({ length: 5 }).map((_, idx) => (
                        <tr key={idx} className="animate-pulse">
                          <td className="py-4 px-4 sm:px-6"><div className="h-4 bg-slate-200 rounded w-36" /></td>
                          <td className="py-4 px-4"><div className="h-3.5 bg-slate-100 rounded w-56" /></td>
                          {tieneColumnaConteo && <td className="py-4 px-4 text-center"><div className="h-4 bg-slate-100 rounded-full w-10 mx-auto" /></td>}
                        </tr>
                      ))
                    ) : errorCarga ? (
                      <tr>
                        <td colSpan={tieneColumnaConteo ? 3 : 2} className="py-12 text-center">
                          <div className="max-w-xs mx-auto flex flex-col items-center">
                            <div className="w-10 h-10 rounded-xl bg-rose-50 text-rose-500 flex items-center justify-center mb-2.5"><AlertTriangle className="w-5 h-5" /></div>
                            <p className="text-xs font-bold text-slate-800">Error al cargar categorías</p>
                      ...
                      <tr>
                        <td colSpan={tieneColumnaConteo ? 3 : 2} className="py-14 text-center">
                          <div className="max-w-sm mx-auto flex flex-col items-center justify-center">
                            <div className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-400 flex items-center justify-center mb-3"><Layers className="w-6 h-6" /></div>
                            <h3 className="text-sm font-bold text-slate-800">No se encontraron categorías</h3>
                            <p className="text-xs text-slate-500 mt-1 mb-4 text-center">
                              {busqueda ? "No hay resultados para la búsqueda ingresada." : "Aún no has registrado categorías en el catálogo."}
                            </p>
                            {busqueda ? (
                              <Button variant="outline" size="sm" onClick={() => setBusqueda("")} className="text-xs">Limpiar búsqueda</Button>
                            ) : tienePermisoEscritura ? (
                              <Link href="/categorias/nuevo">
                                <Button size="sm" className="text-xs gap-1.5 shadow-sm shadow-brand/20">
                                  <Plus className="w-3.5 h-3.5" /><span>Registrar primera categoría</span>
                                </Button>
                              </Link>
                            ) : null}
                          </div>
                      ...
                      categoriasFiltradas.map((cat) => {
                        ...
                        return (
                          <tr key={cat.id} className={`hover:bg-slate-50/80 transition-colors group ${isSelected ? "bg-brand/5" : ""}`}>
                            {/* Nombre — clickeable */}
                            <td className="py-3.5 px-4 sm:px-6">
                              <button
                                type="button"
                                onClick={() => setCategoriaSeleccionada(isSelected ? null : cat)}
                              ...
                              >
                                ...
                                <span className={`font-semibold transition-colors ${isSelected ? "text-brand" : "text-slate-900 group-hover:text-brand"}`}>
                                  {cat.nombre}
                                </span>
                              </button>
                            </td>
                            <td className="py-3.5 px-4 text-slate-600">
                              {cat.descripcion ? (
                                <span className="line-clamp-2">{cat.descripcion}</span>
                              ) : (
          ...
          <div className="py-3 px-4 sm:px-6 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 bg-slate-50/50">
            <span>Total: <strong className="text-slate-800 font-bold">{categoriasFiltradas.length}</strong> {categoriasFiltradas.length === 1 ? "categoría registrada" : "categorías registradas"}</span>
          </div>
        </div>
