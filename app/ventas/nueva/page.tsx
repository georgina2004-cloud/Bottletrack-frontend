...
export default function NuevaVentaPOSPage() {
  ...
  // =========================================================================
  // ESTADOS DEL BUSCADOR DE PRODUCTOS (COLUMNA IZQUIERDA)
  // =========================================================================
  const [busqueda, setBusqueda] = useState<string>("");
  const [categoriaSeleccionada, setCategoriaSeleccionada] = useState<string>("");
  const [productos, setProductos] = useState<Producto[]>([]);
  const [isLoadingProductos, setIsLoadingProductos] = useState<boolean>(true);
  const [errorCargaProductos, setErrorCargaProductos] = useState<string | null>(null);
  ...
  const fetchCatalogo = useCallback(
    async (termino: string, catId?: string) => {
      try {
        ...
        console.error("Error al consultar productos para POS:", err);
  ...
  // Efecto con debounce para búsqueda y filtro en tiempo real
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchCatalogo(busqueda, categoriaSeleccionada);
    }, 300);

    return () => clearTimeout(timer);
  }, [busqueda, categoriaSeleccionada, fetchCatalogo]);

  // 1. Carga inicial de catálogo de categorías para mapeo de sugerencias
  useEffect(() => {
    let isMounted = true;
  ...
  const handleAgregarPresentacionAlCarrito = (
    ...
    if (unidadesUsadas + equiv > producto.stock_actual) {
      setErrorCobro(
        `Stock insuficiente para agregar ${presentacion.nombre} (${equiv} u.). Disponibles: ${
          producto.stock_actual - unidadesUsadas
        } base.`
      );
      return;
    }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            <div className="lg:col-span-7 space-y-3.5">
              ...
              {sugerencias.length > 0 ? (
                <div className="bg-amber-50/60 rounded-2xl border border-amber-200/80 p-3.5 shadow-xs transition-all animate-in fade-in duration-200">
                    ...
                    {sugerencias.map(({ producto: prod, motivo }) => {
                      ...
                      return (
                        <div
                          ...
                          className={`min-w-[165px] sm:min-w-[180px] max-w-[200px] shrink-0 bg-white rounded-xl border p-2.5 flex flex-col justify-between transition-all duration-200 ${
                            !tieneStock
                              ? "opacity-50 border-slate-200 bg-slate-50"
                              : stockAlcanzado
                              ? "border-amber-300 bg-amber-50/20"
                              : "border-amber-200/80 hover:border-amber-400 hover:shadow-xs"
                          }`}
                        >
                          <div className="mb-2">
                            {/* Motivo de la sugerencia */}
                            <span
                              className="text-[10px] font-semibold text-amber-800 bg-amber-50 border border-amber-200/60 px-1.5 py-0.5 rounded-md inline-block max-w-full truncate mb-1"
                              title={motivo}
                            >
                              {motivo}
                            </span>
                ...
                <div className="bg-amber-50/40 rounded-2xl border border-dashed border-amber-200/70 p-3 text-xs text-amber-900/80 flex items-center gap-2.5">
                  <span className="p-1 rounded-md bg-amber-100/80 text-amber-700 shrink-0">
                    <Sparkles className="w-3.5 h-3.5" />
                  </span>
                  <span>
                ...
                {errorCargaProductos ? (
                  <div className="py-16 text-center text-slate-400">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => fetchCatalogo(busqueda)}
            ...
            <div className="lg:col-span-5 space-y-3.5">
              <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-4 flex flex-col justify-between">
                ...
                <div className="space-y-2.5 mb-3.5 p-3 rounded-xl bg-slate-50/70 border border-slate-200/80">
                  {/* Cliente */}
                  <div>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1 flex items-center gap-1.5">
                      ...
                      <span>Descuento fijo ($) (opcional)</span>
                    </label>
                    <input
                      type="number"
                      min="0"
                ...
                <div className="border-t border-slate-200 pt-3 space-y-2 text-xs">
                  ...
                  <div className="border-t border-slate-200 pt-2.5 flex items-baseline justify-between text-sm font-bold text-slate-900">
